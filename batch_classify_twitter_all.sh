...
cleanup() {
    ...
    PIDS=$(pgrep -f "classify_twitter_flood_relevance" 2>/dev/null)
    if [ ! -z "$PIDS" ]; then
        echo "   终止 classify_twitter_flood_relevance 进程..."
        kill -TERM $PIDS 2>/dev/null
        sleep 2
        # 强制终止仍在运行的进程
        REMAINING=$(pgrep -f "classify_twitter_flood_relevance" 2>/dev/null)
        if [ ! -z "$REMAINING" ]; then
...
# 创建日志文件
LOG_FILE="batch_classify_twitter_$(date +%Y%m%d_%H%M%S).log"
echo "📝 日志文件: $LOG_FILE"
echo ""
...
START_TIME=$(date +%s)
echo "==========================================" | tee -a "$LOG_FILE"
echo "🚀 开始批量处理 Twitter flood CSV 文件" | tee -a "$LOG_FILE"
...
for csv_file in "${CSV_FILES[@]}"; do
    ...
    # 运行分类脚本（同时输出到终端和日志文件）
    # 默认并发数改为2，避免连接错误
    if python3 classify_twitter_flood_relevance.py "$csv_file" --start-idx 0 --max-concurrent 2 2>&1 | tee -a "$LOG_FILE"; then
        echo ""
