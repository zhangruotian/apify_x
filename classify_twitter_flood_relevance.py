#!/usr/bin/env python3
"""
使用 Ollama Qwen VLM 模型对 Twitter 数据进行洪水相关性标注
"""
...
class OllamaVLMClassifier:
    ...
    def _build_prompt(
        ...
        if has_images:
            prompt_parts = [
                "Analyze if this Twitter post is flood-related. Consider ALL sources (text, images) and make a COMPREHENSIVE judgment.",
                "",
            ...
            prompt_parts = [
                "Analyze if this Twitter post is flood-related based on TEXT ONLY (no images available).",
                "",
                "Return TRUE if floods/flooding is the PRIMARY content:",
                "- Text: discussions of floods, impacts, events",
                "- News/reporting about flooding",
                "- Political/social commentary where flooding is MAIN topic",
                "- Explicit mentions of flood-related events, damage, or impacts",
                "",
                "Return FALSE if:",
                "- Only passing mention (not main topic)",
                "- Unrelated content using water words metaphorically",
                "- General weather discussion without flood focus",
                "",
                "Analysis: Determine if flooding is the PRIMARY subject based on the text content.",
                "",
                "Strong indicators (TRUE): Explicit flood mentions, flood hashtags (#flood, #bangladeshfloods), detailed flood discussion, flood-related events or impacts.",
                "",
                "Weak indicators (consider context): Brief mention, generic water-related terms, metaphorical flood terms.",
                ""
            ]
        
        # 添加文本信息
        if tweet_text:
            prompt_parts.append(f"Text: {tweet_text}")
            prompt_parts.append("")
        
        # 显示图片数量信息（如果有图片）
        if has_images:
            if total_images and total_images > len(images_base64):
                visual_info = f"Visual: {len(images_base64)} image(s) attached (selected from {total_images} total images)."
            else:
                visual_info = f"Visual: {len(images_base64)} image(s) attached."
            prompt_parts.append(visual_info)
            prompt_parts.append("")
...
async def process_csv_async(
    ...
    """
    异步处理 Twitter CSV 文件，添加洪水相关性标注（支持并发批量处理）
    ...
    """
    print("🌊 Starting Twitter flood relevance classification (Async Mode)")
    ...
    try:
        ...
        for idx, row in rows_to_process.iterrows():
            ...
            if not text or str(text).strip() == "" or str(text).strip().lower() == 'nan':
                ...
                df.at[idx, reason_column] = "No text or images available"
                continue
            
            # 如果没有图片，仍然基于文本进行判断（不跳过）
            
            # 限制图片数量：如果大于3张，任选3张；如果小于3张，有多少选多少
            total_images = len(image_paths)
            max_images = 3
            if total_images > max_images:
                # 随机选择3张
                image_paths_limited = random.sample(image_paths, max_images)
            else:
                # 使用所有图片（如果没有图片，则为空列表）
                image_paths_limited = image_paths
            
            # 创建任务（即使没有图片也会创建，基于文本判断）
            tasks.append((idx, text, image_paths_limited, image_paths))
        ...
        async def process_single_task(task_data):
            ...
            idx, text, image_paths_limited, image_paths_all = task_data
            row_num = idx + 1
            ...
            async with semaphore:
                try:
                    ...
                    prompt, images_base64 = classifier._build_prompt(
                        text=text,
                        image_paths=image_paths_limited,
                        project_root=project_root,
                        total_images=len(image_paths_all)  # 传递总数用于提示
                    )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="使用 Ollama VLM 对 Twitter 数据进行洪水相关性标注",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
示例用法:
  # 处理整个 CSV 文件
  python classify_twitter_flood_relevance.py twitter/assam_flood/csvs/filtered_assam_flood_tweets_20240501_20240801_with_local_paths_20250721_172531.csv
  
  # 指定输出文件
  python classify_twitter_flood_relevance.py input.csv -o output.csv
  
  # 从第10行开始处理，最多处理20行
  python classify_twitter_flood_relevance.py input.csv --start-idx 10 --max-rows 20
  
  # 不使用断点续传（重新处理所有行）
  python classify_twitter_flood_relevance.py input.csv --no-resume
  
  # 自定义并发数（加速处理）
  python classify_twitter_flood_relevance.py input.csv --max-concurrent 5
        """
    )
